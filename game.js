const games = {
  floor1: {
    id: "floor1",
    title: "🍭 1F キッズゲーム",
    desc: "はじめてでも遊びやすい、<br>明るくてにぎやかなゲームコーナー。",
    className: "floor-1",
    items: [
      {
        title: "ポキポキきゅうり",
  ...
  floor2: {
    id: "floor2",
    title: "🌇 2F 放課後ゲーム",
    desc: "夕方、寄り道、ちょっと変。<br>中学生気分でのぞくゲームフロア。",
    className: "floor-2",
    items: [
      {
        title: "バカなカバの大冒険",
  ...
  basement: {
    id: "basement",
    title: "🌀 実験部屋",
    desc: "まだ開発中。<br>いつか変な試作ゲームが置かれるかも。",
    className: "floor-b1",
    items: []
  },
  ...
  drink: {
    id: "drink",
    title: "🥤 ドリンクバー",
    desc: "のみもの系ゲームはこちら。<br>飲みすぎ注意の休憩コーナー。",
...
function getAllGames() {
  return Object.values(games).flatMap(section => section.items);
}
...
if (randomBtn) {
  randomBtn.addEventListener("click", playRandomGame);
}
...
function setCounterText(elementId, value, digits, errorText) {
...
function updateCounter(url, elementId, digits, errorText) {
  fetch(url)
    .then(res => {
      if (!res.ok) {
    ...
    .then(data => {
      setCounterText(elementId, data.value, digits, errorText);
    })
    .catch(() => {
      const element = document.getElementById(elementId);
...
updateCounter(
  "https://countapi.mileshilliard.com/api/v1/hit/afoolhippo/kabagame-total",
  "totalCounter",
  5,
  "ERROR"
);
...
updateCounter(
  `https://countapi.mileshilliard.com/api/v1/hit/afoolhippo/${todayKey}`,
  "todayCounter",
  4,
  "ERR"
);